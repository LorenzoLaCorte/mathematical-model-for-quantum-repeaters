...
The random variable $T_{n}$ \textbf{represents the waiting time} necessary to achieve a link between the endpoints of a segment joining $2^{n}$ nodes. 
...
Plugging in the expressions for $\Pr(K_n = k)$ (Equation~\ref{eq:pdf_swap_steps}) and recalling that the sum of $k$ copies of $M_n$ can be expressed as a convolution (see Appendix~\ref{section:convolution}), we get
...
One example of operation that can be efficiently computed in the Fourier domain is the convolution of two or more probability distributions (see Appendix~\ref{section:convolution_distributions}), which can be expressed as a product of their Fourier transforms. This is a direct consequence of the linearity of the Fourier transform, which converts convolutions into element-wise multiplication.
