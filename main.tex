...
We define the joint distribution that one attempt succeeds or fails and takes time $t$ as
\begin{align}
    P_s(t) &:= \Pr(M = t, Y = 1) = \sum_{t_A, t_B : \max(t_A, t_B) + t_c = t} \Pr(T_A = t_A, T_B = t_B) \cdot p(t_A, t_B) \\
    P_f(t) &:= \Pr(M = t, Y = 0) = \sum_{t_A, t_B : \max(t_A, t_B) + t_c = t} \Pr(T_A = t_A, T_B = t_B) \cdot [1 - p(t_A, t_B)]
\end{align}
...
\begin{align}
    \Pr(T_{out} = t) = \sum_{k=1}^{\inf} \left[ \left( \ast_{j=1}^{k-1} P_f \right) \ast P_s \right]
\end{align}
...
First, we compute the average Werner parameter of the output link of one attempt, given that it succeeds and finishes at time $t$

\[
Ws(t) = \frac{\sum_{\substack{t_A, t_B : \\ \max(t_A, t_B) + t_c = t}} \Pr(T_A = t_A, T_B = t_B) \cdot [p \cdot w_{\text{out}}](t_A, t_B)}{P_S(t)}.
\] % todo: check if the formula is correct

Next, we take a weighted average of Ws over all possible sequences of failed attempts, followed by a single successful attempt:

where $P_S*W_S$ is the weighted average.
