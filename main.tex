...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.2\linewidth]{images/dist.png}
...
From previus results, we found that the total waiting time is given by $\sum_{j=1}^{k} m_{j}$, the sum of the duration of the production of the lost pairs (see Equation \ref{eq:pdf_waiting_time_swap}). % todo: check is the sum and not the convolution 
...
For entanglement distillation, the success probability $p_{dist}$ depends on the Werner parameters; in particular
\begin{equation}
...
To compute he waiting time distribution we consider the generation time of a successful or failed attempt separately and use the joint distribution of $M$ and $Y$. % todo: maybe ref

We define the joint distribution that one attempt succeeds or fails and takes time $t$ as % todo: maybe sep. in two lines the max
\begin{align}
...
We iterate over all possible combinations of the input links' generation time $t_A$, $t_B$ that leads to a waiting time $t$ for this attempt.

The sum of the waiting time for all attempts can be obtained by
\begin{align}
    \Pr(T_{out} = t) = \sum_{k=1}^{\infty} \left[ \left( \ast_{j=1}^{k-1} P_f \right) \ast P_s \right]
\end{align}

The sum over $k$ considers all the possible numbers of attempts, convoluting, for each case, $k-1$ unsuccessful attempts and one successful one.

Neither $P_f$ or $P_s$ characterizes a random variable since they are joint distributions including $Y$. 
That is to say, $Ps$ and $Pf$ do not sum up to 1.
Instead, we have
\begin{equation}
...
The full computation goes as follow.
...
Is important to remark that $w_{out}$ is the Werner parameter of the output link if the distillation attempt was successful, and $p_{dist}$ is the success probability of the distillation protocol.
...
The protocol is visualized in \ref{fig:mixed_protocol_sketch}.
...
\begin{figure}[h]
    ...
    \caption{The secret key rate is $3.186 * 10^{-3}$.} % todo: introduce secret key rate
...
The Fourier transform is a linear map.

Moreover it converts convolutions into element-wise multiplication

As a consequence, taking the Fourier transform of both sides of Equation ?? yields

\paragraph*{Full Derivation}

The probability distribution of the total waiting time \( T_{\text{out}} \) for all attempts is given by:
\begin{equation}
...
For $k$ attempts, the productory \(\prod_{j=1}^{k-1} \mathcal{F}[P_f^{(j)}]\) represents the convolution of the failure distributions in the Fourier domain. 

From the assumption made, these distributions \( P_f^{(j)} \) are identical, so:
\begin{equation}
    \prod_{j=1}^{k-1} \mathcal{F}[P_f^{(j)}] = \left( \mathcal{F}[P_f] \right)^{k-1}
\end{equation}

Using the identity for the sum of a geometric series \(\sum_{k=1}^{\infty} x^{k-1} = \frac{1}{1-x}\), the equation simplifies to:
\begin{equation}
    \mathcal{F}[\Pr(T_{\text{out}} = t)] = \sum_{k=1}^{\infty} \left( \left( \mathcal{F}[P_f] \right)^{k-1} \cdot \mathcal{F}[P_s] \right)
\end{equation}

Recognizing the geometric series in the sum, we obtain:
\begin{equation}
...
% Applying the inverse Fourier transform to return to the time domain
...
The transformation of Equation ?? to Fourier space involves the same concepts and yields
% todo: include it
...
Equation ?? and ?? can also be moved to the Fourier space as follows.

The derivation is simple: both expressions can be written in Fourier space by substituting $P_s = p_{swap} \cdot m(t)$ and $Pf = (1 - pswap) \cdot m(t)$ in ?? and ??.

As a result, the waiting time distribution in the Fourier space is:
% include it

and the Werner parameter is 
% include it
