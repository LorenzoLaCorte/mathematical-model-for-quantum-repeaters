...
% Make sure all elements of Li Table are fully covered
% Add a discussion on classical communication and use it consistenly
% Make sure (out) or (n+1) pedixes are used consistently
% Avoid discussing cutoffs
...
We consider Werner states, which simplifies the analysis of the quality of the entanglement. % todo: here I need to cite the Werner states concept in the background
The random variable $W_{n}$ \textbf{represents the quality of the entanglement} produced, quantified by the Werner parameter of the output link. It is related to its fidelity $F_{n}$ by % todo: here I need to cite the fidelity concept in the background
\begin{equation}
    F_{n} = \frac{1 + 3 W_{n}}{4}
\end{equation}
and thus represents \textbf{the quality of the entanglement}.

% We may also use a different notation, $T_{\text{out}}$ or $W_{\text{out}}$, to refer to the waiting time for the output link of a PROTOCOL-UNIT (generation, swapping, distillation).
% In general, these two notations represent the same object, as one step of the protocol is represented by the execution of a PROTOCOL-UNIT.
...
The first step in the repeater chain protocol is the generation of elementary entanglement between two nodes. % todo: cite the concept of elementary entanglement in the background
...
Considering two entangled pairs, respectively with Werner parameters $w_A$ and $w_B$, the output Werner parameter, if we do not consider decoherence, will be % todo: for the thesis, cref werner parameter
\begin{equation}
...
% todo: when mergin, add reference to the background
...
% TODO: maybe divide the fraction and the sum: 1 / ... * (sum) 
...
% To calculate $\Pr(T_{n+1} = t)$, we need three joint distributions
% \begin{enumerate}
...
% \begin{equation}
%     \Pr(T_{n+1} = t) = \mathcal{F}^{-1} \left[ \frac{\mathcal{F}[p_\text{swap} \cdot m(t)]}{1 - \mathcal{F}[(1 - p_\text{swap}) \cdot m(t)]} \right](t)
% \end{equation}
% and
% \begin{equation}
%     W_{n+1} = \mathcal{F}^{-1} \left[ \frac{\mathcal{F}[p_\text{swap} \cdot m(t) \cdot W_s]}{1 - \mathcal{F}[(1 - p_\text{swap}) \cdot m(t)]} \right] \frac{1}{\Pr(T_{n+1}=t)}(t)
% \end{equation}
% where $m$ is defined in Equation~\ref{eq:pdf_waiting_time_pair_convolution}.

\appendix

\chapter{Mathematical Background}\label{app:math_background} % todo: when merging, divide math. background for q.mechanics and for q.repeaters
